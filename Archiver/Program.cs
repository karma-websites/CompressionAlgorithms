...
internal class Program
{
    ...
    {
        Console.WriteLine("Archiver: RLE + BWT + MTF + RLE + HUFFMAN");
    ...
    private static void CompressFile(string dataFileName, string archFileName)
    {
        ...
        stopwatch.Restart();
        byte[] rleData2 = Rle.Compress(mtfData);
        stopwatch.Stop();
        Console.WriteLine($"{"RLE size data: ", -50} {rleData2.Length} byte. \tWorking time: {stopwatch.ElapsedMilliseconds} ms");

        stopwatch.Restart();
        byte[] arch = Huffman.Compress(rleData2, out double averageLength);
        stopwatch.Stop();
    ...
    private static void DecompressFile(string archFileName, string dataFileName)
    {
        ...
        Console.WriteLine($"{$"Inverse RLE size data: ", -50} {mtfData.Length} byte. \tWorking time: {stopwatch.ElapsedMilliseconds} ms");
        ...
        Console.WriteLine($"{$"Inverse BWT size data: ", -50} {rleData2.Length} byte. \tWorking time: {stopwatch.ElapsedMilliseconds} ms");

        stopwatch.Restart();
        byte[] data = Rle.Decompress(rleData2);
